...
# Ключи метанных статей (TbArticle).
KEY_ARTICLE_HUB = 'HUB'       # Ключ для хранения информации о том, что статья является хабом (содержит ссылки на другие статьи)
...
class ValidateMatchType(IntEnum):
    ...
    FIND_IN_SYNONYM = 2   # Точное совпадение в синонимах
    EXACT_SYNONYM_MATCH = 3   # Точное совпадение синонимов проверяемой записи с синонимами остальных записей
    PARTIAL_MATCH = 4      # Схожесть: частичное совпадение (например, часть названия артиста или лейбла в других записях).
                           # Основного поля и синонимами проверяемой записи и всех остальных записей.
